...
// قائمة القبائل المستخدمة في نافذة الاختيار (بدون كلمة "قبيلة")
const saudiTribes = [
...
/**
 * ⚠️ إضافة: اختيار تلقائي عبر شات البث — بدون تدخل الستريمر. أثناء فتح
 * نافذة اختيار (إقصاء أو إرجاع)، يُبنى خريطة "رقم البطاقة/اسم القبيلة"
 * → نفس دالة الاختيار التي كان الضغط اليدوي يستدعيها بالضبط. أول تعليق
 * تيك توك حقيقي يطابق أي منهما يُنفَّذ تلقائياً (أول تطابق يفوز).
 *
 * ⚠️ الضغط اليدوي **يبقى يعمل أيضاً** بالتوازي (احتياطي) — لم يُحذَف.
 */
var _activeChatSelectionMap = null;

function clearChatSelectionHandlers() {
    _activeChatSelectionMap = null;
}

function registerChatSelectionHandlers(entries) {
    // entries: [{ number, tribeName, onSelect }]
    var map = {};
    entries.forEach(function (entry) {
        map[String(entry.number)] = entry.onSelect;
        map[normalizeArabicText(entry.tribeName)] = entry.onSelect;
    });
    _activeChatSelectionMap = map;
}
...
function handleChatCommentForSelection(text, commenterName) {
    ...
    if (handler) {
        clearChatSelectionHandlers(); // أول تطابق يفوز — يمنع أي تنفيذ مزدوج من تعليقات لاحقة
        handler();
    }
}
window.AGP_handleChatComment = handleChatCommentForSelection;
...
// ============================================================
//  سحب العجلة (معطّل حالياً) - العجلة الآن ثابتة دائماً في منتصف
//  الشاشة عبر CSS (position: fixed)، ويمكن تكبيرها أو تصغيرها فقط
//  من متحكم الحجم أعلى يسار الشاشة. تم إبقاء الكود هنا (بدون تفعيل)
//  في حال رغب الستريمر مستقبلاً بإعادة تفعيل السحب اليدوي.
// ============================================================
if (false && wheelDragLayer) {
...
function renderPlayerListSidebar() {
    ...
    // إظهار/إخفاء الشريط الجانبي تلقائياً - لا يظهر إلا بعد بدء الجولة فعلياً
    // (وليس بمجرد إضافة اللاعبين للعجلة قبل الضغط على "بدء الجولة")
    if (gameSidebar) {
        gameSidebar.classList.toggle('hidden', players.length === 0 || !gameStarted);
    }
...
// ============================================================
//  إغلاق نافذة اختيار القبيلة يدوياً (زر X) - يغلق النافذة دون اتخاذ أي
//  قرار (لا إقصاء ولا إرجاع) ويعيد زر التدوير جاهزاً للجولة القادمة مباشرة
// ============================================================
function closeDecisionModal() {
    clearSelectionTimer();
    clearChatSelectionHandlers();
...
if (modalCloseBtn) {
    modalCloseBtn.addEventListener('click', closeDecisionModal);
}
...
function openEliminationModal() {
    ...
    targetSlots.forEach((targetPlayer, i) => {
        ...
        card.innerHTML = `<span class="tribe-card-number">${i + 1}</span><span class="tribe-card-name">${tribeLabels[i]}</span>`;
    ...
    registerChatSelectionHandlers(targetSlots.map((targetPlayer, i) => ({
        number: i + 1,
        tribeName: tribeLabels[i],
        onSelect: () => handleEliminationChoice(targetPlayer ? targetPlayer.uid : null)
    })));
...
function openReentryModal() {
    ...
    emptyCard.innerHTML = `<span class="tribe-card-number">${totalEliminated + 1}</span><span class="tribe-card-name">${tribeLabels[totalEliminated]}</span>`;
    emptyCard.addEventListener('click', handleReentryEmptyChoice, { once: true });
    tribesContainer.appendChild(emptyCard);

    registerChatSelectionHandlers(
        shuffledSlots.map((originalIdx, i) => ({
            number: i + 1,
            tribeName: tribeLabels[i],
            onSelect: originalIdx !== null ? (() => handleReentryChoice(originalIdx)) : handleReentryEmptyChoice
        })).concat([{
            number: totalEliminated + 1,
            tribeName: tribeLabels[totalEliminated],
            onSelect: handleReentryEmptyChoice
        }])
    );
